...
template <typename t>
typename AVLTree <t> :: node* AVLTree <t> :: removeNode (node* node, const t& item)
{
    if (node == nullptr)
    {
        return node;
    ...
    if (node->data > item)
    {
        node->LChild = removeNode(node->LChild, item);
    }
    else if (node->data < item)
    {
        node->RChild = removeNode(node->RChild, item);
    }
    else
    {
        ...
        else
        {
            ...
            node->data = temp->data;
            node->RChild = removeNode(node->RChild, temp->data);
        }
    }
...
void AVLTree <t> :: deleteItem (const int &index)
{
    ...
    nameRoot = removeNode(nodeToBeDeleted, this->arr[index-1]);

    nodeToBeDeleted = findNode(priceRoot, this->arr[index-1]);
    priceRoot = removeNode(nodeToBeDeleted, this->arr[index-1]);
